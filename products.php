...
// Query to fetch details from coupons table based on the search term
$sql = "SELECT t1.*, 
               CASE 
        ...
        WHERE t1.coupon_id NOT IN (SELECT coupon_id FROM confirmed WHERE confirmed_at <= NOW() - INTERVAL 24 HOUR)";

if (!empty($search_term)) {
    $stmt = $con->prepare("SELECT t1.*, 
                                  CASE 
                                      WHEN t2.coupon_id IS NOT NULL THEN 'Unavailable'
                                      ELSE 'Available'
                                  END AS status
            ...
            <div class="row">
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        $couponClass = $row['status'] === 'Unavailable' ? 'unavailable' : '';
